...
def compute_destinations():
    """ Calculates  e, f_alpha_des, and alpha_des values for all persons"""
...
neighbor_interact
neighbor_move_ped
compute_single_destination
move_pedestrian_single
set_virtual_destination
move_pedestrians_virtual
ogive_acceleration
ogive_perturbation
wrap_to_pi
hybrid_model
"""    
#------------------------------------------------------------------------------#
def neighbor_interact(p,angle_update = None, angle_range = None):
    """compute neighborhood interaction for ped p"""
    """
       spdacc: speed acceleration for ped p
       angspd: angular spd for ped p
    """ 
    spdacc = 0
    angspd = 0
    ...
    angsum = 0
    nped = 0
    listped = list(range(n))
    listped.remove(p)
    spdp = np.sqrt(v[0][p]*v[0][p]+v[1][p]*v[1][p])
    
    if angle_range == None:
        angle_range = np.pi/2
    
    for others in listped:
        xdiff = x[0][others]-x[0][p]
        ydiff = x[1][others]-x[1][p]
        dist = np.sqrt(xdiff*xdiff+ydiff*ydiff)
        spdo = np.sqrt(v[0][others]*v[0][others]+v[1][others]*v[1][others])
        
        anglep = np.arccos(v[0][p]/spdp)
        if v[1][p] < 0:
            anglep = -anglep
        angleo = np.arccos(v[0][others]/spdo)
        if v[1][others] < 0:
            angleo = -angleo
        if angle_update:
            anglep = angle_update
        
        angle = np.arccos(xdiff/dist)
        
        if ydiff < 0:
            angle = -angle
            
        # filter pedestrians that are in the sight range (5m, 180 degree in the front)
        if (dist <= d_max) and (abs(anglep - angle) <= np.pi/2) and (abs(angleo-anglep)<= angle_range):
            nped += 1
            w_i = consta/(np.exp(constw*dist)+consta) #coupling strength
            spddiffsum += w_i*(spdo - spdp)
            angsum += w_i*np.sin(angleo - anglep)
        
    if nped != 0:        
        spdacc = spddiffsum*constc/nped 
        angspd= angsum*constk/nped
    return spdacc,angspd
            
def neighbor_move_ped(p,spdacc,angspd,collisionwall=None):
    """ Moves pedestrian p forward in time by time_step based on calculated spdacc and angspd values"""
    #Global values being saved
    global v, x 
    spdp = np.sqrt(v[0][p]*v[0][p]+v[1][p]*v[1][p])
    anglep = np.arccos(v[0][p]/spdp) 
    if v[1][p] < 0:
        anglep = -anglep
    # spdpdash = np.sqrt(v_full[0][p][-2]*v_full[0][p][-2] + v_full[1][p][-2]*v_full[1][p][-2])
    # anglepdash = np.arccos(v_full[0][p][-2]/spdpdash) 
    # if v_full[1][p][-2] < 0:
    #     anglepdash = -anglepdash
    # angspd = (anglep - anglepdash)/time_step
    # angle_update = anglep + angspd*time_step + 0.5*angacc*time_step
    # _,angacc_update = neighbor_interact(p, angle_update = angle_update)
    # angspd = angspd + 0.5*(angacc + angacc_update)*time_step
    v[0,p] = (spdp + spdacc* time_step) * np.cos(anglep + angspd* time_step)
    v[1,p] = (spdp + spdacc* time_step) * np.sin(anglep + angspd* time_step)
    
    if collisionwall:
        [abcx,abcy] = compute_wall_acceleration_neighbor(p)
        print("v {}".format(v[:,p]))
        print("a due to wall is {}".format([time_step*abcx,time_step*abcy]))
        #acceleration
        a = np.zeros_like(v)
        a[0,p] = abcx
        a[1,p] = abcy
        ...
        #update
        v[:,p] = v[:,p] + a[:,p] * time_step
        
    x[:,p] = x[:,p] + v[:,p] * time_step    
    
#------------------------------------------------------------------------------#

def compute_single_destination(i):
    """ Calculates v_des, f_alpha_des, and alpha_des values for person i"""
    global alpha_des, f_alpha_des, v_des, gap, contact_p,in_field, d_h
    ...
    v_des[i] = min(v_0[i],d_h/(1*tau))
    
def move_pedestrians_single(i):
    """ Moves pedestrian i forward in time by time_step based on calculated v_des and alpha_des values"""
    #Global values being saved
    global v, x

    #acceleration due to body collisions - needs to computed before moving the pedestrians to ensure both people colliding feel the force
    [abcx,abcy] = compute_bodycollision_acceleration(i)
       
    #acceleration
    a = np.zeros_like(v)
    #print("destination acc is {}".format((np.cos(alpha_des[i])*v_des[i]-v[0,i])/tau))
    #print("collision acc is {}".format(abcx))
    a[0,i] = (np.cos(alpha_des[i])*v_des[i]-v[0,i])/tau + abcx
    a[1,i] = (np.sin(alpha_des[i])*v_des[i]-v[1,i])/tau + abcy

    #update
    v[:,i] = v[:,i] + a[:,i] * time_step
    x[:,i] = x[:,i] + v[:,i] * time_step    
    
#--------------------------------------------------------------------------------------------    

def set_virtual_destination(i,alpha,speed):    
    """ set v_des and alpha_des values for virtual ped i"""
    global v_des , alpha_des
    v_des[i] = speed
    alpha_des[i] = alpha
    
def move_pedestrians_virtual(k):
    """ Moves virtual pedestrian k forward in time by time_step based on calculated v_des and alpha_des values"""
    #Global values being saved
    global v, x
    
    a = np.zeros_like(v)
    
    a[0,k] = (np.cos(alpha_des[k])*v_des[k]-v[0,k])/tau
    a[1,k] = (np.sin(alpha_des[k])*v_des[k]-v[1,k])/tau

    #update
    v[:,k] = v[:,k] + a[:,k] * time_step
    x[:,k] = x[:,k] + v[:,k] * time_step     

def ogive_acceleration(i,tspan,miu,sigma,speed):
    """
    Generate ogive acceleration for ped i from rest
    Parameters
    ----------
    i : perdestrain
    tspan : time span
    miu : mean of ogive function
    sigma : s.d. of ogive function
    speed : resultant increase in speed
    """
    
    global v, x
    speednow = speed * (ss.norm.cdf(t,miu,sigma)-ss.norm.cdf(0,miu,sigma))/(ss.norm.cdf(tspan,miu,sigma)-ss.norm.cdf(0,miu,sigma))
    
    #update
    v[1,i] =  speednow
    x[:,i] = x[:,i] + v[:,i] * time_step

def ogive_perturbation(i,t0,tspan,miu,sigma,speed,angle):
    """
    Generate ogive perturbation for ped i
    Parameters
    ----------
    i : perdestrain
    t0 : initial time
    ...
    miu : mean of ogive function
    sigma : s.d. of ogive function
    speed : resultant change in speed
    angle : resultant change in angle in degrees, left minus right plus
    """
    
    global v, x
    speednow = speed * (ss.norm.cdf(t-t0,miu,sigma)-ss.norm.cdf(0,miu,sigma))/(ss.norm.cdf(tspan,miu,sigma)-ss.norm.cdf(0,miu,sigma))
    anglenow = angle * (ss.norm.cdf(t-t0,miu,sigma)-ss.norm.cdf(0,miu,sigma))/(ss.norm.cdf(tspan,miu,sigma)-ss.norm.cdf(0,miu,sigma))
    #update
    if t == 0:
        vt0 = v_full[:,i]
    else:
        vt0 = v_full[:,i,int(t0/time_step)]
    speednow = speednow + np.sqrt(vt0[0]**2 + vt0[1]**2)
    anglenow = np.arctan2(vt0[1],vt0[0]) - np.radians(anglenow)
    v[0,i] = speednow * np.cos(anglenow)
    v[1,i] = speednow * np.sin(anglenow)
    x[:,i] = x[:,i] + v[:,i] * time_step

#----------------------------------------------hybrid functions-------------------------------------------------

def compute_wall_acceleration_neighbor(i):
    """ Returns acceleration in [x,y] directions caused by body collisions
        for person i current positions of persons """
...
def hybrid_model(kappa, return_collision = False):
    """ Moves pedestrians forward in time by time_step by hybrid model"""
    #Global values being saved
    global v, x, aw_full 
    ...
    abcx = np.zeros(n)#contact acceleration x
    abcy = np.zeros(n)#contact acceleration y
    spdacc = np.zeros(n)#speed acceleration
    angspd = np.zeros(n)#angular speed
    spd = np.zeros(n)#speed
    angle =  np.zeros(n)#heading
    v_neighbor = np.zeros(n)#resultant speed in neighborhood model 
    angle_neighbor = np.zeros(n)#resultant heading in neighborhood model
    collision = np.zeros(n)
    
    #acceleration due to body collisions - needs to computed before moving the pedestrians to ensure both people colliding feel the force
    for p in range(n):
        [abcx[p],abcy[p]] = compute_bodycollision_acceleration(p)
        
        if np.linalg.norm([abcx[p],abcy[p]]) > 0:
            collision[p] = 1
       
        spdacc[p],angspd[p] = neighbor_interact(p)
        #print("spdacc for pedestrian {} is {}".format(p,spdacc[p]))
        #print("angspd for pedestrian {} is {}".format(p,np.degrees(angspd[p])))
        if (angspd[p]-0)>np.radians(0.5):
            
            spd[p] = np.sqrt(v[0][p]*v[0][p]+v[1][p]*v[1][p])
            angle[p] = np.arccos(v[0][p]/spd[p]) 
            if v[1][p] < 0:
                angle[p] = -angle[p]
        
            v_neighbor[p] = spd[p] + spdacc[p]* time_step
            v_des[p] = min(v_0[p], v_0[p]*kappa + (1-kappa)*v_neighbor[p],d_h[p]/(1*tau))
            #print("v_des for pedestrian {} is {}".format(p,v_des[p]))
            
            angle_neighbor[p] =  wrap_to_pi(angle[p] + angspd[p]*time_step)
            #print("angacc for pedestrian {} is {}".format(p,np.degrees(angacc[p])))
            #print("alpha_said for pedestrian {} is {}".format(p,np.degrees(alpha_des[p])))
            #print("alpha_neighbor for pedestrian {} is {}".format(p,np.degrees(angle_neighbor[p])))
            alpha_des[p] = kappa * alpha_des[p] + (1-kappa)*angle_neighbor[p]
            #print("alpha_des for pedestrian {} is {}".format(p,np.degrees(alpha_des[p])))
            
    a = np.zeros_like(v)#resultant acceleration
