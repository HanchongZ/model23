...
def bypass_funcs(dx_ij,dy_ij,alpha,d_max):
    ...
    return -1
#------------------------------------------------------------------------------#
def f(alpha,i,params,params_a,debug=False):
    """ Compute the minimum distance to collision in this direction"""

    rsum_i,dx_i,dy_i,quad_C2_i,in_field = params
    v_xi,v_yi,dv_x,dv_y,quad_A2 = params_a
...
def compute_alpha_des(i, params,display_falpha = False, display_distancefunc = False):
    ...
    if (n<-3):
        ...
        for index in range(len(alphas)):
            params_a = (v_xi_a[index],v_yi_a[index],dv_x[index],dv_y[index],quad_A2[index])
            f_alphas[index] = f(alphas[index],i,params,params_a)
...
def compute_bodycollision_acceleration(i):
    ...
    return [axt+axw, ayt+ayw, axw, ayw]
...
#        params = [rsum[i,:],dx[i,:],dy[i,:],quad_C2[i,:],in_field[0],None,None,None]
    out = Parallel(n_jobs=num_cores)(delayed(compute_alpha_des)(i,[rsum[i,:],dx[i,:],dy[i,:],quad_C2[i,:],in_field[i]]) for i in range(n))
    for i in range(n):
...
def compute_destinations():
    ...
    for i in range(n):
        ...
        in_field.append(ifield[0])
#        params = [rsum[i,:],dx[i,:],dy[i,:],quad_C2[i,:],in_field[0],None,None,None]
        alpha_des[i],f_alpha_des[i],contact_w[i,:]= compute_alpha_des(i,[rsum[i,:],dx[i,:],dy[i,:],quad_C2[i,:],in_field[i]])
...
def move_pedestrians():
    ...
    for i  in range(n):
        [abcx[i],abcy[i],axw[i],ayw[i]] = compute_bodycollision_acceleration(i)

    aw_full = np.dstack((aw_full, [axw, ayw]))
