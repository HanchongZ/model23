# -*- coding: utf-8 -*-
"""
Created on Fri Feb 17 00:04:23 2023
...
#------------------------------------------------------------------------------#
p = 3 #near 3 case
ntrial_l = 10*np.arange(1,31)
spdstd = np.zeros(len(ntrial_l))
meanspd = np.zeros(len(ntrial_l))

for e in range(len(ntrial_l)):
    finalspd = np.zeros(ntrial_l[e])
    for f in range(ntrial_l[e]):
        model.reset_model()    
        ...
        for q in np.linspace(start,end,num):#0.5s to 1.5s
            finalspd[f] += np.sqrt(model.v_full[0,0,int(q)]**2 + model.v_full[1,0,int(q)]**2)/num
    
    spdstd[e] = np.std(finalspd)
    meanspd[e] = np.mean(finalspd)

plt.figure(figsize=(8,6))
#plt.plot(ntrial_l,spdstd,"x--") 
# Plot the data with error bars
plt.errorbar(ntrial_l, meanspd, yerr=spdstd, fmt='o', capsize=4)   
