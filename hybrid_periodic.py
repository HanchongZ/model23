...
for u in range(len(n_l)):
    ...
    for i in range(len(kappa_l)):
        for _ in range(ntrials):
            ...
            avgspd_vals[u,i] += np.average(avgspdlist)/ntrials            
            collision_vals[u,i] += np.average(collisionlist)/ntrials
   ...
   #------------------------------------------------------------------------------#
    ...
    # plt.savefig('results/hybrid_periodic_'+str(n_l[u])+'.png')
...
plt.grid(alpha = 0.5)
# plt.show()
plt.savefig('results/c_occupancy_avgspd.png')
...
plt.grid(alpha = 0.5)
# plt.show()
plt.savefig('results/c_occupancy_colno.png')
