...
model.walls[:,0] = np.array([ 1, 0, 1.75/2, -7.88/2, 7.88/2])
# wall y = -1.75/2
model.walls[:,1] = np.array([ 1, 0, -1.75/2, -7.88/2, 7.88/2])
#------------------------------------------------------------------------------#
...
#------------------------------------------------------------------------------#
fig = plt.figure()
plot_pub = plt.plot( data[:,1], data[:,0], 'r', label = 'Published Results')
...
plt.axvline(1.75/2,color = 'k')
plt.axvline(-1.75/2,color ='k')
plt.legend(loc=1)
# model.plot_current_positions(fig, ['blue','red'])
plt.savefig('results/corridor_2peoplev.png')
